...
<ul>
  ...
  <li><strong>Export / import (move or back up prompts):</strong> Click <strong>Export</strong> and choose a JSON file, Microsoft OneDrive, or Google Drive. Cloud backups contain all 20 prompt slots and labels in the active profile, are encrypted in the browser with the password you choose, and are stored in your own account's private app folder. A new cloud export replaces only the previous prompt backup for that same service; it does not overwrite the API-key backup or other files. To restore, click <strong>Import</strong>, choose the same service, and enter the password. Import always replaces the prompts in the <strong>active</strong> profile and warns you if the backup came from another profile.</li><br>
